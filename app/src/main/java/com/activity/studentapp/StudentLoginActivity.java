...
public class StudentLoginActivity extends AppCompatActivity {
    ...
    private void showForgotPasswordDialog() {
        ...
        com.google.android.material.textfield.TextInputLayout tilEmail = dialog.findViewById(R.id.tilEmail);
        ...
        com.google.android.material.button.MaterialButton btnSendReset = dialog.findViewById(R.id.btnSendReset);
        ...
        btnSendReset.setOnClickListener(v -> {
            ...
            // Check if email is from Gmail domain (based on admin app enrollment)
            if (!email.toLowerCase().endsWith("@gmail.com")) {
                tilEmail.setError("Please use your Gmail email address (@gmail.com)");
                etEmail.requestFocus();
                return;
            }
            ...
            // First check if email exists in Firestore students collection
            db.collection("students")
                    .whereEqualTo("email", email)
                    .get()
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful() && task.getResult() != null && !task.getResult().isEmpty()) {
                            // Email exists in students collection, proceed with password reset
                            proceedWithPasswordReset(email, btnSendReset, dialog);
                        } else {
                            // Email not found in students collection
                            tilEmail.setError("This email address is not registered with any student account");
                            etEmail.requestFocus();
                        }
                    })
                    .addOnFailureListener(e -> {
                        showToast("Error checking email registration: " + e.getMessage());
                    });
        });
    ...
    private void proceedWithPasswordReset(String email, com.google.android.material.button.MaterialButton btnSendReset,
        ...
        mAuth.fetchSignInMethodsForEmail(email)
                .addOnCompleteListener(fetchTask -> {
                    Log.d("StudentLogin", "fetchSignInMethodsForEmail completed. Success: " + fetchTask.isSuccessful());

                    if (fetchTask.isSuccessful()) {
                        java.util.List<String> signInMethods = fetchTask.getResult().getSignInMethods();
                        Log.d("StudentLogin", "Raw signInMethods result: " + signInMethods);
                        Log.d("StudentLogin", "signInMethods is null: " + (signInMethods == null));
                        if (signInMethods != null) {
                            Log.d("StudentLogin", "signInMethods size: " + signInMethods.size());
                            Log.d("StudentLogin", "signInMethods content: " + signInMethods.toString());
                        }

                        if (signInMethods != null && !signInMethods.isEmpty()) {
                            Log.d("StudentLogin", "Email " + email + " is registered in Firebase Auth with methods: "
                                    + signInMethods);
                            // Email exists in Firebase Auth, proceed with password reset
                            ...
                            Log.w("StudentLogin", "Email " + email
                                    + " is not registered in Firebase Auth. signInMethods: " + signInMethods);
                        }
                    } else {
                        ...
                        Log.e("StudentLogin", "Error checking email registration: " + errorMessage,
        ...
        mAuth.sendPasswordResetEmail(email)
                .addOnCompleteListener(task -> {
                    ...
                    if (task.isSuccessful()) {
                        ...
                        Log.d("StudentLogin", "Password reset email sent successfully to: " + email);
                        dialog.dismiss();
                    } else {
                        String errorMessage = task.getException() != null ? task.getException().getMessage()
                                : "Unknown error";
                        showToast("Failed to send password reset email: " + errorMessage);
                        Log.e("StudentLogin", "Password reset email failed: " + errorMessage);
                    }
                });
    }
